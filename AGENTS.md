...
- Agent-driven company setup must use `scripts/setup_memoryendpoints_company.py`, which writes the one-time company master credential to `.local-secrets/memoryendpoints-company-master.json` relative to the project root without printing it. Do not claim setup is complete until that file exists. Normal agents use their bound agent credential; read the company master only for a company-administration operation allowed by published agent policy. If the default file is missing, stop and treat autonomous setup as incomplete; check only an explicitly configured governed secret store. Do not require a human, scan outside configured paths, or request, echo, or log the raw credential.
