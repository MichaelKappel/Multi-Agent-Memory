...
[human release log](https://multiagentmemory.com/releases/), or the
...
For agent-driven setup on the server computer, use `python scripts/setup_memoryendpoints_company.py --project-root .`. The helper uses the machine name, Windows user, and project-folder name as editable labels unless explicit label arguments are supplied. Those labels are convenience metadata, never authentication. The helper checks both destinations before the non-idempotent setup request, writes the company master to the standard project file, writes the owner-recovery secret to a separate user recovery file, and prints only redacted confirmation.
