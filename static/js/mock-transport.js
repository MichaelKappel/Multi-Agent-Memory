  ...
  function create(options) {
    ...
    function response(descriptor, requestOptions) {
      ...
      if (key === "POST /api/matm/memory-events/submit") {
        ...
        return {ok:true,event:clone(memory),canonicalMemoryEventId:memoryId,persisted:true,visibleInSearch:true,visibleInReviewQueue:true,visibleInAuditLog:true,operatorSummary:{memoryEventId:memoryId,scope:memory.scope,memoryType:memory.memoryType,reviewStatus:"pending",firewallDecision:"accepted",rawPayloadExposed:false,rawCredentialExposed:false},mockData:true};
      }
      ...
      if (key === "GET /api/matm/audit-log") {
        var auditItems=[{auditId:"mock-audit",action:"tour.session.read",route:"/tour",actorAgentId:agentId,valuesRedacted:true,rawCredentialExposed:false,rawPayloadExposed:false,details:{transport:"session mock transport",persistent:false}}];
        var auditFilters=compactFilters({action:queryValue(query,["action"])});
        var matchedAudit=auditItems.filter(function(item){return !auditFilters.action||item.action===auditFilters.action;}).slice(0,requestedLimit(query,50));
        return {ok:true,schemaVersion:"memoryendpoints.audit_log.v1",items:clone(matchedAudit),count:matchedAudit.length,filters:auditFilters,operatorSummary:{count:matchedAudit.length,filters:auditFilters,actionCounts:countBy(matchedAudit,"action","unknown"),allCredentialsHidden:true,allPayloadsHidden:true,payloadHidden:true,valuesRedacted:true,rawCredentialExposed:false,rawPayloadExposed:false},valuesRedacted:true,rawCredentialExposed:false,rawPayloadExposed:false,mockData:true};
      }
