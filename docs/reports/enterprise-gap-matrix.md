...
| CI status after latest push | `failure` for current observed SHA `7d8fe787a74b`; Could not read GitHub Actions public API: HTTPError status 403. Previous public-safe CI evidence: Could not read GitHub Actions public API: HTTPError status 403. | Resolve GitHub account/Actions blocker, then require a passing CI run for the pushed SHA. |
