...
Report source snapshot: `fae36ec661917dda597b376447ad741f1291d077`. Tracked reports are point-in-time evidence; rerun the no-write live and CI verifiers after a final push to prove the current commit.

## Verified

- Local verification report: `pass`, see `docs/reports/local-verification-report.json`.
- Report freshness: local route report `true`, local route evidence `true`, package report `true`, package evidence `true`, GitHub CI report `false` for current HEAD `fae36ec66191`.
...
- Live latest-code SHA verification snapshot: expected `fae36ec661917dda597b376447ad741f1291d077`, observed `None`, match `false`.
...
- GitHub Actions CI snapshot: `failure`; observed run did not prove code health because `Could not read GitHub Actions public API: HTTPError status 403. Previous public-safe CI evidence: Latest GitHub Actions run failed before workflow steps executed: The job was not started because your account is locked due to a billing issue.`.
...
- GitHub Actions CI: blocked in the tracked snapshot. Could not read GitHub Actions public API: HTTPError status 403. Previous public-safe CI evidence: Latest GitHub Actions run failed before workflow steps executed: The job was not started because your account is locked due to a billing issue.
...
```json
{
  ...
  "reportSourceSha": "fae36ec661917dda597b376447ad741f1291d077",
  "valuesRedacted": true
}
```
