...
Report source snapshot: `0282c8619408dfa0b620de49bb3108ccd34f1eab`. Tracked reports are point-in-time evidence; rerun no-write package, WSGI, live route, live SHA, dogfood, `.uai`, and secret-scan checks after a final push to prove the current commit.
...
- Snapshot freshness at report generation: local route report `true`, package report `true`, GitHub Actions required `false` for snapshot HEAD `0282c8619408`.
...
- Local WSGI route verification: 22 routes, 0 failures, 0 public leak hits.
...
- Live MySQL/MariaDB backend verification: `blocked`; observed backend `file`, configured backend ``, connection verified `false`.
...
- Package verification: status `ready`, 83 planned files, excludes local runtime state and secrets.
- Deploy dry-run: status `ready`, planned files `81`, safe no-op `true`, matches package `true`.
- Secret scan: 116 scanned files, 0 hits.
...
- Latest-code live deployment: blocked or not verified. The recorded upload attempt failed at `None` with `None`; uploaded count was `81`; connection checks `ftps/connection_check_passed/0 uploads, ftp/connection_check_failed/0 uploads`; live source SHA match is `true`.
...
```json
{
  ...
  "liveMysqlBackendVerified": false,
  "multiAgentMemoryLiveDeployed": true,
  "multiAgentMemoryLiveSiteVerified": true,
  "reportSourceSha": "0282c8619408dfa0b620de49bb3108ccd34f1eab",
  "valuesRedacted": true
}
```
