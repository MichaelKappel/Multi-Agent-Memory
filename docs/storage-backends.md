...
SQLite schema convergence is guarded by the application-owned `PRAGMA user_version`. A new or older database is upgraded inside a `BEGIN IMMEDIATE` transaction, and the version marker advances only after every table, column, and index step succeeds. A database created by a newer application version fails closed instead of being modified by older code. Every SQLite DDL change must increment `_SQLITE_SCHEMA_VERSION` in `memoryendpoints/storage.py`; the durable database marker, rather than a process-local path cache, is the authority across worker processes, database replacement, and restore operations.
