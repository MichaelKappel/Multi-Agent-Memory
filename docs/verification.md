...
Use the exact versioned ZIP and adjacent closed release-identity JSON file produced by the truth-ordered workflow in `docs/deployment.md`. The ZIP contains exactly the 16 public files and no embedded manifest; the external JSON describes and hashes it. After FTPS stages the ten non-claim files, run preactivation HTTPS verification while the required tag is still absent:
...
The preactivation verifier requests exactly the ten non-claim routes and must return `nonclaims_live_verified_preactivation`. Only after that proof and the immediately-before-tag dry-run requalification may the exact annotated tag be created and pushed. Final package qualification must then pass, and the final deploy must RETR the ten staged paths before it can STOR and RETR the six claims. After that returns `claims_activated_final`, run final HTTPS verification against the same unchanged ZIP and external manifest:
...
The verifier refuses network access without an explicit phase and both release artifacts. It accepts only the exact base origin `https://multiagentmemory.com`; redirects, final-origin drift, non-200 responses, MIME drift, and byte drift fail closed. Preactivation fails if the required tag exists locally or remotely and verifies exactly ten routes without requesting a claim. Final fails unless the annotated tag's internal name/type/target are canonical, the local and remote tag-object SHA-1 values match, both peel to the package-bound commit, and all 16 routes match. Final success is `full_live_verified_final`. The current live domain must not be treated as final while any required stage, identity, activation, or verification gate fails, and `main` is published only after final HTTPS GO.
