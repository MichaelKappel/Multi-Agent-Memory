...
Use the exact versioned ZIP, adjacent closed release-identity JSON, and target-bound prior-state rollback pair produced by the truth-ordered workflow in `docs/deployment.md`. The forward ZIP contains exactly the 18 public files and no embedded manifest; the external JSON describes and hashes it, binds retired `releases.html`, and defines the 19-path rollback policy. Capture must complete before the first STOR. After FTPS qualifies the unchanged prior state and stages the twelve non-claim files, run preactivation HTTPS verification while the required tag is still absent:
...
The preactivation verifier requests exactly the twelve non-claim routes and must return `nonclaims_live_verified_preactivation`. Only after that proof and the immediately-before-tag dry-run requalification may the exact annotated tag be created and pushed. Final package qualification must then pass. The final deploy must RETR the twelve staged paths, prove all 19 managed paths contain only held-prior or bound-forward bytes, STOR and RETR the six claims, then delete retired `releases.html` and prove it absent. After that returns `claims_activated_final`, run final HTTPS verification against the same unchanged ZIP and external manifest:
...
The verifier refuses network access without an explicit phase and both forward release artifacts. It accepts only the exact base origin `https://multiagentmemory.com`; redirects, final-origin drift, non-200 responses, MIME drift, and byte drift fail closed. Preactivation fails if the required tag exists locally or remotely and verifies exactly twelve routes without requesting a claim. Final fails unless the annotated tag's internal name/type/target are canonical, the local and remote tag-object SHA-1 values match, both peel to the package-bound commit, all 18 canonical routes match, and retired `/releases.html` returns an ordinary `404` without redirect. Final success is `full_live_verified_final` with `fileCount: 18`, `retiredRouteCount: 1`, and `retiredRouteVerifiedCount: 1`. The current live domain must not be treated as final while any required stage, identity, rollback, activation, retired-route, or verification gate fails, and `main` is published only after final HTTPS GO.
